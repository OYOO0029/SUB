...
export class SyncEngine{
 constructor({api,bridge,storage,scope,notion,status=()=>{},validate=validateState,namespace='myWeekSync'}){Object.assign(this,{api,bridge,storage,scope,notion,status,validate,namespace});this.active=true;this.ready=false;this.conflict=false;this.base=null;this.dirty=false;this.sequence=0;this.chain=Promise.resolve();this.timer=null;
  this.metaKey=this.namespace+'Pending:'+scope;
  try{const m=JSON.parse(storage.getItem(this.metaKey)||'null');if(m?.dirty){this.pending=this.validate(m.state);this.base=m.base;this.dirty=true;this.sequence++;if(!same(this.pending,bridge.read()))this.conflict=true}}catch{this.conflict=true;status('error','오류','미전송 기록을 읽지 못했습니다. 백업 후 확인하세요.')}
 }
 ...
 backup(state,reason){this.validate(state);const key=this.namespace+'Backup:'+this.scope+':'+Date.now()+':'+Math.random().toString(36).slice(2);this.storage.setItem(key,JSON.stringify({savedAt:new Date().toISOString(),reason,state}));return key}
 localChanged(){if(!this.active)return;this.pending=this.validate(this.bridge.read());this.dirty=true;this.sequence++;try{this.checkpoint()}catch(e){this.conflict=true;this.status('error','오류','미전송 백업 저장 실패. JSON을 내보내 주세요.');return}clearTimeout(this.timer);this.status('syncing','동기화 중','이 기기에 저장했습니다. 서버 연결을 확인합니다.');this.timer=setTimeout(()=>this.reconcile(),500)}
 ...
 async check(){
  ...
  if(!row){this.ready=false;if(this.conflict)return;
   ...
   let original;try{original=this.validate(JSON.parse(this.bridge.originalCache))}catch{throw Error('기존 Notion 캐시 형식을 확인할 수 없습니다. JSON 백업 후 수동 업로드하세요.')}
   this.backup(original,'notion-original-cache');
   const state=this.validate(this.sequence===0?original:this.bridge.read());this.backup(state,'before-notion-seed');
  ...
  if(!same(row.state,this.bridge.read())){this.backup(this.bridge.read(),'before-server-load');this.bridge.apply(this.validate(row.state))}
  this.base=row.updated_at;this.ready=true;this.checkpoint();this.status('online','동기화됨','서버 최신 일정이 반영되었습니다.');
 }
 async push(row){
  if(!this.active||!this.dirty)return;const sequence=this.sequence,state=this.validate(this.pending);this.status('syncing','동기화 중','서버에 저장합니다.');
